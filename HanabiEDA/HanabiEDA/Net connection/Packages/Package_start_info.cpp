...
typedef struct package_data
{
	...
	card_hanabi	client_hand[TOTAL_HAND_CARDS];
	card_hanabi	server_hand[TOTAL_HAND_CARDS];
}package_data;
#pragma pack(pop)
...
bool Package_start_info::set_info(const card client_hand[TOTAL_HAND_CARDS], const card server_hand[TOTAL_HAND_CARDS])
{
	package_data* p_data = (package_data*)raw_data;
	const card((*hands[2])) = {client_hand , server_hand };
	card_hanabi(*package_hands[2]) = { p_data->client_hand ,p_data->server_hand };
	bool cards_ok = true;
...
bool Package_start_info::get_info(card client_hand[TOTAL_HAND_CARDS], card server_hand[TOTAL_HAND_CARDS])
{
	package_data* p_data = (package_data*)raw_data;
	card((*hands[2])) = { client_hand , server_hand };
	const card_hanabi(*package_hands[2]) = { p_data->client_hand ,p_data->server_hand };
	bool cards_ok = true;
	card_color_t color;
...
bool Package_start_info::is_raw_data_valid(const char * data, size_t data_length)
{
	...
	{
		const card_hanabi(*hands[2]) = { p_data->client_hand,p_data->server_hand };
