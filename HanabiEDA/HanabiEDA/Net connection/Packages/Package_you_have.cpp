...
bool Package_you_have::get_clue(card *clue)
{
	...
	else if (p_data->number != ONE &&
			p_data->number != TWO && p_data->number != THREE &&
			p_data->number != FOUR && p_data->number != FIVE)
	{
		switch (p_data->color)
		{
		case BLUE:
...
bool Package_you_have::is_raw_data_valid(const char * data, size_t data_length)
{
	...
	if (data_length == sizeof(package_data) && p_data->header == YOU_HAVE_ID)
		return (p_data->color == RED || p_data->color == YELLOW ||
			p_data->color == GREEN || p_data->color == BLUE ||
			p_data->color == WHITE || p_data->number == ONE ||
			p_data->number == TWO || p_data->number == THREE || 
			p_data->number == FOUR || p_data->number == FIVE);
	return false;
}